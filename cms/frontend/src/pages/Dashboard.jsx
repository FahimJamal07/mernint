            ...
            <div className="d-flex flex-grow-1 overflow-hidden">
                ...
                <main className="flex-grow-1 p-5" style={{ overflowY: 'auto' }}>
                    <div className="d-flex justify-content-between align-items-center mb-5">
                        <h2 className="fw-bold text-dark" style={{textShadow: '0 2px 4px rgba(255,255,255,0.5)'}}>Available Courses</h2>
                        <span className="text-white badge bg-dark opacity-50">Live Catalog</span>
                    </div>
                                        ...
                                        <div className="card h-100 glass-card border-0">
                                            ...
                                            <div className="card-body">
                                                <div className="d-flex justify-content-between align-items-start mb-2">
                                                    ...
                                                    <span className="badge bg-light text-dark border shadow-sm">${course.price}</span>
                                                </div>
                                                <p className="text-muted small mb-3">{course.description}</p>
                                                
                                                <div className="d-flex align-items-center gap-2 mb-3">
                                                    <div className="progress flex-grow-1" style={{height: '6px', backgroundColor: 'rgba(0,0,0,0.1)'}}>
                                                        <div 
                                                            className={`progress-bar ${seatsLeft < 3 ? 'bg-danger' : 'bg-success'}`} 
                                                            style={{width: `${(enrolledCount / (course.seats || 10)) * 100}%`}}
                                                        ></div>
                                                    </div>
                                                    <span className="small text-muted fw-bold" style={{fontSize: '0.75rem'}}>
                                                        {seatsLeft} left
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="card-footer bg-transparent border-top-0 p-3 pt-0">
                                                {user.role === 'admin' ? (
                                                    <div className="d-flex gap-2">
                                                        <button onClick={() => handleDelete(course._id)} className="btn btn-outline-danger flex-grow-1 btn-sm bg-white">Delete</button>
                                                        <button onClick={() => navigate(`/admin/course-edit/${course._id}`)} className="btn btn-outline-warning flex-grow-1 btn-sm bg-white">Edit</button>
                                                    </div>
                                                ) : (
                                                    isEnrolled ? (
                                                        <button disabled className="btn btn-light text-success w-100 fw-bold border">✓ Enrolled</button>
                                                    ) : isFull ? (
